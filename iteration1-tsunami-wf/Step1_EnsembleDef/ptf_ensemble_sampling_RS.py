...
def compute_ensemble_sampling_RS(**kwargs):
    ...
    #Nsamp1=np.ones(RS_samp_run)*10
    #Nsamp2=np.ones(RS_samp_run)*100
    Nsamp3=np.ones(RS_samp_run)*1000
    Nsamp4=np.ones(RS_samp_run)*5000
    #Nsamp5=np.ones(RS_samp_run)*10000
    #Nsamp6=np.ones(RS_samp_run)*20000
    #Nsamp=np.concatenate((Nsamp3,Nsamp4), axis=None)
    ...
    for N in Nsamp:
        ...
        N=int(N)
        NBS= int(round(TotProbBS_all*N)) ### NBS: number of scenarios sampled from the BS ensemble
...
def bs_probability_scenarios(**kwargs):
    ...
    for i in range(Num_Samp):
       ...
       deptlim=50.0*1000.0
       bounds[0,0]=xyz[0]-distlim
       bounds[0,1]=xyz[0]+distlim
       bounds[1,0]=xyz[1]-distlim
       bounds[1,1]=xyz[1]+distlim
       bounds[2,0]=1000.0+(v_hwidth*math.sin(dip_real_val_rad))/math.sin(math.pi/4)
       bounds[2,1]=deptlim 
       Nsteps=1000
