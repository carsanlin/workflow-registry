...
def compute_ensemble_sampling_MC(**kwargs):
    ...
    Nsamp1=np.ones(MC_samp_run)*10
    Nsamp2=np.ones(MC_samp_run)*100
    Nsamp3=np.ones(MC_samp_run)*1000
    Nsamp4=np.ones(MC_samp_run)*5000
    Nsamp5=np.ones(MC_samp_run)*10000
    #Nsamp=np.concatenate((Nsamp2,Nsamp4), axis=None)
    sampled_ensemble = {}
...
def find_nearest(array, value):
    ...
    if (value-array[idx])>0 and idx<len(array)-1:
        idx=idx+1
