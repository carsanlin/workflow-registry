...
def compute_hazard_curves(**kwargs):
    ...
    if (probability_scenarios['PScomputedYN'] == True and short_term['PS_computed_YN'] == True and len(probability_scenarios['par_scenarios_ps'][:,0]) > 0):
    ...
    if (probability_scenarios['BScomputedYN'] == True and short_term['BS_computed_YN'] == True and len(probability_scenarios['par_scenarios_bs'][:,0]) > 0):
    ...
    if(short_term['PS_computed_YN'] == True and short_term['BS_computed_YN'] == True):
    ...
    elif(short_term['PS_computed_YN'] == True and short_term['BS_computed_YN'] == False):
    ...
    elif(short_term['PS_computed_YN'] == False and short_term['BS_computed_YN'] == True):
        hazards['hazard_curves_at_pois']      = hazards_bs['hazard_curves_bs_at_pois']
        hazards['hazard_curves_at_pois_mean'] = hazards_bs['hazard_curves_bs_at_pois_mean']
        hazards['ps'] = False
        hazards['bs'] = hazards_bs
...
def select_hazard_curves_for_bs_k_scenarios(**kwargs):
    ...
    # Open and read hdf5-curves file
    infile = h_curve_files['gl_bs'][relevant_file_scenario]
    ...
    ItmpInd               = data[:, nr_scenarios] > 0

    S = np.ma.masked_values(InputIntensityMeasure, 0)
    ...
    for i in range(0, int(nr_cpu_allowed)):
        ...
        results_scenarios_loop.append(scenario_loop.remote(idxs        = secondary_loop_idxs,
                                                           cpu         = i,
                                                           probsel     = probsel,
                                                           ItmpInd     = ItmpInd,
                                                           S           = S,
    ...
    results = ray.get(results_scenarios_loop)
...
def hazard_curves_bs(**kwargs):
    ...
    for k in range(len(relevant_scenarios_bs)):
        ...
        result_ids.append(select_hazard_curves_for_bs_k_scenarios.remote( nr                = relevant_scenarios_bs[k],
                                                                cfg               = Config,
                                                                args              = args,
                                                                hazards           = hazards,
                                                                pois              = pois,
