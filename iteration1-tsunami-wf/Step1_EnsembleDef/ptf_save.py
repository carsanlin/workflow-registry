...
#    al_fcp_table  = root_name + '_' + Config.get('save_ptf','table_alert_level_fcp')
    ...
    if IS_samp_scen>0:
        hazard_curves_IS = Config.get('save_ptf','hazard_curves_IS')
        save_d['hazard_curves_IS']    = hazard_curves_IS
    if RS_samp_scen>0:
        hazard_curves_RS = Config.get('save_ptf','hazard_curves_RS')
        save_d['hazard_curves_RS']    = hazard_curves_RS
    if ND_samp>0:
        hazard_curves_ND = Config.get('save_ptf','hazard_curves_ND')
        save_d['hazard_curves_ND']    = hazard_curves_ND
...
def save_ptf_dictionaries(**kwargs):
    ...
    if(ptf != False):
        ...
        if IS_samp_scen>0:
           new_ens_IS = {}
           rangeis=len(ptf['new_ensemble_IS'])
           hazard_curves_is={}
           for Nid in range(rangeis):
               new_ens_IS['%d'%Nid] = ptf['new_ensemble_IS'][Nid]
               hazard_curves_is['%d'%Nid]         = ptf['hazard_curves_IS_%d'%Nid]
        if RS_samp_scen>0:
           new_ens_RS = {}
        ...
        if ND_samp>0:
           rangend=len(ptf['ND_probability_scenarios'])
           hazard_curves_nd={}
           for Nid in range(rangend):
               hazard_curves_nd['%d'%Nid]         = ptf['hazard_curves_ND_%d'%Nid]
    ...
    if (ptf != False and status != 'no_message'):
        ...
        if(format_hc == 'npy'):
            ...
            if IS_samp_scen>0:
               for Nid in range(rangeis):
                   np.save(save_dict['event_path'] + os.sep + save_dict['hazard_curves_IS'] + '_' + '%d'%Nid,   hazard_curves_is['%d'%Nid],    allow_pickle=True)
            if RS_samp_scen>0:
               for Nid in range(rangers):
                   np.save(save_dict['event_path'] + os.sep + save_dict['hazard_curves_RS'] + '_' + '%d'%Nid,   hazard_curves_rs['%d'%Nid],    allow_pickle=True)
            if ND_samp>0:
               for Nid in range(rangend):
                   np.save(save_dict['event_path'] + os.sep + save_dict['hazard_curves_ND'] + '_' + '%d'%Nid,   hazard_curves_nd['%d'%Nid],    allow_pickle=True)
        ...
        #
            ...
            if IS_samp_scen>0:
               for Nid in range(rangeis):
                   h5file = save_dict['event_path'] + os.sep + save_dict['hazard_curves_IS'] + '_' + '%d'%Nid + '.hdf5'
                   hf = h5py.File(save_dict['event_path'] + os.sep + save_dict['hazard_curves_IS'] + '_' + '%d'%Nid + '.hdf5', 'w')
                   hf.create_dataset('hazard_curves_at_pois', data=hazard_curves_is['%d'%Nid]['hazard_curves_at_pois'])
                   hf.create_dataset('hazard_curves_at_pois_mean', data=hazard_curves_is['%d'%Nid]['hazard_curves_at_pois_mean'])
                   hf.create_dataset('hazard_curves_bs_at_pois', data=hazard_curves_is['%d'%Nid]['bs']['hazard_curves_bs_at_pois'])
                   hf.create_dataset('hazard_curves_bs_at_pois_mean', data=hazard_curves_is['%d'%Nid]['bs']['hazard_curves_bs_at_pois_mean'])
                   hf.create_dataset('tsunami_intensity_name', data=hazard_curves_is['%d'%Nid]['tsunami_intensity_name'])
                   if (new_ens_IS['%d'%Nid]['nr_ps_scenarios'] > 0):
                       hf.create_dataset('hazard_curves_ps_at_pois', data=hazard_curves_is['%d'%Nid]['ps']['hazard_curves_ps_at_pois'])
                       hf.create_dataset('hazard_curves_ps_at_pois_mean', data=hazard_curves_is['%d'%Nid]['ps']['hazard_curves_ps_at_pois_mean'])
                   else:
                       hf.create_dataset('hazard_curves_ps_at_pois', data=np.array([0.0]))
                       hf.create_dataset('hazard_curves_ps_at_pois_mean', data=np.array([0.0]))
                   hf.close()
