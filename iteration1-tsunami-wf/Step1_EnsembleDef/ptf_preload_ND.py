...
def load_PSBarInfo_ND(**kwargs):
    ...
    PSBarInfo_Dict_mod={}
    PSBarInfo_Dict_mod['BarPSperModel']={}
    PSBarInfo_Dict_mod['BarPSperModelReg']={}
    PSBarInfo_Dict_mod['BarPSperModelDepth']={}
    
    for i in range(9):
        PSBarInfo_Dict_mod['BarPSperModel'][i]={}
        PSBarInfo_Dict_mod['BarPSperModel'][i]=PSBarInfo_Dict['BarPSperModel'][i]
        PSBarInfo_Dict_mod['BarPSperModelReg'][i]={}
        PSBarInfo_Dict_mod['BarPSperModelReg']=PSBarInfo_Dict['BarPSperModelReg'][i]
        PSBarInfo_Dict_mod['BarPSperModelDepth'][i]={}
        PSBarInfo_Dict_mod['BarPSperModelDepth']=PSBarInfo_Dict['BarPSperModelDepth'][i]

    return PSBarInfo_Dict_mod
...
#        np.save(pois_npy, POIs)
    ...
    if os.path.isfile(discretization_npy) and args.preload != 'Yes' and args.preload_discretization != 'Yes':
        ...
        # Importance sampling loading for magnitude
        num_mag_bs=9
        num_mag_ps=9
        Mw=event_parameters['mag']
        sig=event_parameters['MagSigma']

        # Importance sampling loading for strike/dip/rake
        rake=[]
        num_rake=4
        # number of rake sampling: 1 sampling corresponding to 36 combinations of strike and dip
        # num_rake=4 -> total of 144 str/dip/rake combination
        weights=(0.1,0.4,0.1,0.4) # weight of 0°, 90°, 180°, 270°

        dip1=[10,30,50,70,90]
        dip2=[10,30,50,70]
        strike1=[22.5,67.5,112.5,157.5]
        strike2=[202.5,247.5,292.5,337.5]

        ############################ Importance sampling of mag #################################
        newid=[]
        newvaltmp=np.sort(np.random.normal(loc=Mw, scale=sig, size=num_mag_bs))
        newval=newvaltmp
        for i in range(len(newvaltmp)):
           closestmag=min(discretizations['BS-1_Magnitude']['Val'], key=lambda x:abs(x-newvaltmp[i]))
           newval[i]=closestmag
        newvalu,newvali = np.unique(newval,return_counts=True)
        for i in newvalu:
           newid.append('M'+ str(int(i*100)))
        
         
        discretizations['BS-1_Magnitude']['ID']=newid
        discretizations['BS-1_Magnitude']['Val']=newvalu.tolist()
        discretizations['BS-1_Magnitude']['Dup']=newvali.tolist()
        discretizations['PS-1_Magnitude']['ID']=newid
        discretizations['PS-1_Magnitude']['Val']=newvalu.tolist()
        discretizations['PS-1_Magnitude']['Dup']=newvali.tolist()       
        print(newvalu,newvali,newid)        
 
        ############################ Importance sampling of rake #################################
        ...
        rakeinit=[0,90,180,270]
        dip1=[10,30,50,70,90]
        dip2=[10,30,50,70]
        strike1=[22.5,67.5,112.5,157.5]
        strike2=[202.5,247.5,292.5,337.5]
        comb1=np.array(np.meshgrid(strike1,dip1)).T.reshape(-1,2)
        comb2=np.array(np.meshgrid(strike2,dip2)).T.reshape(-1,2)
        comb=np.array([]).T.reshape(-1,2)
        
        for i in range(num_rake):
           comb=np.concatenate((comb,comb1,comb2),axis=0)
        
        for i in range(len(comb)):
           raketmp=roll(weights)
           closestrake=min(rakeinit, key=lambda x:abs(x-raketmp))
           rake.append(closestrake)
        
        finalcomb=np.concatenate((comb,(np.array([rake])).transpose()),axis=1)
        
        newid=[]
        newval=[]
        for i in finalcomb:
           newid.append('S'+str(int(i[0])).rjust(3, '0')+'D'+str(int(i[1])).rjust(3, '0')+'R'+str(int(i[2])).rjust(3, '0'))
           newval.append(str(i[0])+' '+str(i[1])+' '+str(i[2]))
        
        discretizations['BS-4_FocalMechanism']['ID']=newid
        discretizations['BS-4_FocalMechanism']['Val']=newval
...
#        os.makedirs(pyptf_probability_models_dir)
    ...
    return LongTermInfo, ModelsProb_Region_files
